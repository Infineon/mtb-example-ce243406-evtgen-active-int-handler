# PDL: Event generator active interrupt handler

This code example demonstrates how to use an event generator to trigger an interrupt handler while the device is operating in active power mode. The result is indicated by an LED blinking at 500 ms intervals.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce243406-evtgen-active-int-handler)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce243406-evtgen-active-int-handler](https://github.com/Infineon/mtb-example-ce243406-evtgen-active-int-handler)" application with the desired name "EVTGEN_int_handler" configured for the *KIT_T2G_C-2D-6M_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G_C-2D-6M_LITE --app-id mtb-example-ce243406-evtgen-active-int-handler --user-app-name  EVTGEN_int_handler --target-dir "C:/mtb_projects"
   ```
...
3. After programming, the application starts automatically. If the interrupt handler has started correctly, the user LED1 will light up at 500 ms intervals, confirming normal operation

> **Note:** If initialization fails, the user LED1 will blink at 150 ms intervals.
...
This code example demonstrates how to use the event generator in active power mode to trigger an interrupt handler and output the result to user LED1.

Details of the event generator configuration is as follows:

- Event generator interval ticks

       g_evtgen_interval_ticks = (frequency_tick_hz * EVTGEN_BLINK_PERIOD_MS) / 1000UL;
       /* Compare Value = ( 1000000 * 500 ) / 1000 = 1000000 */
...
- Event generator configuration
       ...
       .frequencyRef = frequency_ref_hz,   /* CLK_HF3 = 100 MHz */
       .frequencyLf = Cy_SysClk_ClkLfGetFrequency(),   /* CLK_LF = 32.768 kHz */
       .frequencyTick = frequency_tick_hz,   /* Counter Tick = 1000000 */
       .ratioControlMode = CY_EVTGEN_RATIO_CONTROL_SW,   /* Software Control */
       .ratioValueDynamicMode = CY_EVTGEN_RATIO_DYNAMIC_MODE0,   /* Ratio Dynamic Mode 0 */ 
       };
   
       cy_stc_evtgen_struct_config_t evtgen_struct_config =
       {
      
       .functionalitySelection = CY_EVTGEN_ACTIVE_FUNCTIONALITY,   /* Active Power mode */
       .triggerOutEdge = CY_EVTGEN_EDGE_SENSITIVE,   /* Trigger Mode = Edge Sensitive */
       .valueActiveComparator = g_evtgen_interval_ticks,   /* Compare Value = 1000000 */
       .valueDeepSleepComparator = 0UL,   /* Not using DeepSleep mode */
       };
...
Document title: *CE243406* – *PDL: Event generator active interrupt handler* 
